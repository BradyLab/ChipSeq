...
def get_group(ko,wt,window_size,outdir):
    ...
    for gap in range(0,6):
        ...
        sicer_cmd = "sh /usr/local/bin/SICER_V1.1/SICER/SICER-df-rb.sh {0} {1} {2} {3} 0.1 0.001 -o {4}".format(ko,wt,window_size,gap_size,outdir)
        print >>sys.stderr, sicer_cmd
        commands.getoutput(sicer_cmd)

def main(ko,wt,outdir,window_size=200):
    get_group(ko,wt,window_size,outdir)
